...
describe("ArtistPage", () => {
  ...
  beforeEach(() => {
    ...
    axiosMock
      .onGet(
        "https://api.spotify.com/v1/artists/test-artist-id/albums?include_groups=album%2Csingle",
      )
  ...
  it("renders artist info, albums, and tracks correctly", async () => {
    ...
    await waitFor(() => {
      ...
      const releaseDateElements = screen.getAllByText("Release date:", {
        exact: false,
      });
      expect(releaseDateElements[0].parentElement).toHaveTextContent(
        "2020-01-01",
      );
      expect(releaseDateElements[1].parentElement).toHaveTextContent(
        "2022-05-15",
      );
